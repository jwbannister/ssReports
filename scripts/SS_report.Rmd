...
for (i in 1:splits){
    ...
    split_events[[i]] <- events[((33*(i-1))+1):(33*i), ]
    split_events[[i]] <- split_events[[i]][complete.cases(split_events[[i]]), ]
    cat("<p style=\"font-size: 10pt;\">")
    pandoc.table(select(split_events[[i]], 1:4), split.table=Inf)
    cat("</p>")
}
...
if (comment_file %in% comment_list[ , 2]){
    ...
    if ((nrow(split_events[[splits]]) + comment_length)<33){
        cat(" \n\n## Comments \n")
        ...
        split_comments <- list()
        split_comments[[1]] <- comment_lines[1:(32-nrow(split_events[[splits]]))]
        comment_splits <- ceiling((comment_length-length(split_comments[[1]]))/33)
        for (i in 2:(comment_splits+1)){
            split_comments[[i]] <- comment_lines[(length(split_comments[[i-1]])+1):
                                                 (length(split_comments[[i-1]])+33)]
        ...
        for (k in 1:(comment_splits+1)){
            ...
            for (j in 1:length(split_comments[[i]])){
                cat(" \n", split_comments[[i]][j], " \n", sep="")
            }
        }
    }
