...
if (comment_file %in% comment_list[ , 2]){
    ...
    if ((nrow(split_events[[splits]]) + comment_length)<32){
        cat(" \n\n## Comments \n")
        ...
        split_comments <- list()
        split_comments[[1]] <- comment_lines[1:(31-nrow(split_events[[splits]]))]
        comment_splits <- 
        ceiling((comment_length-length(split_comments[[1]]))/32) + 1
        marker <- length(split_comments[[1]])
        for (i in 2:(comment_splits)){
            split_comments[[i]] <- comment_lines[(marker+1):(marker+32)]
