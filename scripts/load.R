...
query1 <- paste0("SELECT i.deployment, ", 
                 ...
                 "CASE WHEN (datetime - '1 second'::interval)::date < ",
                 "'2017-12-01'::date THEN m.net_rad ELSE m.solar_rad END ",
                 "AS solar_rad, ", 
                 ...
                 "AS invalid_wd, ",
                 "CASE WHEN (datetime - '1 second'::interval)::date < ",
                 "'2017-12-01'::date THEN ",
                 "flags.field_is_invalid(m.deployment_id, 118, m.datetime) ",
                 "ELSE flags.field_is_invalid(m.deployment_id, 337, m.datetime) ",
                 "END AS invalid_rad, ",
